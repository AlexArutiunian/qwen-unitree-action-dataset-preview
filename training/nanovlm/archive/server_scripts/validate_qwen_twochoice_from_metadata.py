...
def main():
    ...
    for i, r in tqdm(df.iterrows(), total=len(df), desc="Qwen two-choice"):
        try:
            ...
            prompt = f"""
You are analyzing a robot navigation camera image.
...
- Choose only A or B.
- Focus on physical evidence in the image.
- If glass or mirror-like reflections reveal a visible room behind them, that does not automatically mean there is an open gap.
- If a closed transparent/reflective panel is physically in front of the opening, choose the blocked/panel option.
- Return only valid JSON.

Required JSON format:
{{"answer":"A or B", "explanation":"one short sentence explaining the visible evidence"}}
""".strip()
    ...
    out.to_csv(out_dir / "qwen_twochoice_results.csv", index=False)
